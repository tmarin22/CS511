...
do_join(State, Ref, ChatName) ->
    ChatMap = State#cl_st.con_ch,
    case maps:is_key(ChatName,ChatMap) of
    	true -> {err,State};
    	false -> whereis(server)!{self(), Ref, join, ChatName},
    			receive 
    				{CRPID, Ref, connect, History}->
    				NewChatMap = maps:put(ChatName,CRPID,ChatMap),
    				{History, State#cl_st{con_ch=NewChatMap}}		
    			end
    end.
    	...
    	true -> whereis(server)!{self(),Ref,leave,ChatName},
    			receive 
    				{_Server, Ref, ack_leave} ->
    						NewChatMap = maps:remove(ChatName,ChatMap),
    						{ok,State#cl_st{con_ch=NewChatMap}}
    			end
    end.
...
do_new_nick(State, Ref, NewNick) ->
    case State#cl_st.nick of
    	...
    	_ -> whereis(server)!{self(),Ref,nick,NewNick},
    		receive
    			{_Server, Ref, err_nick_used} -> {err_nick_used, State};
    			{_Server, Ref, ok_nick} -> {ok_nick, State#cl_st{nick=NewNick}}
    		end
    end. 
